...
#pragma warning disable 612, 618
        ...
        public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, int pageSize, int pageNumber, params OracleParameter[] commandParameters)
        {
            ...
            using (OracleConnection cn = new OracleConnection(connectionString))
            {
                ...
                //call the overload that takes a connection in place of the connection string
                return ExecuteDataset(cn, commandType, commandText, pageSize, pageNumber,null, commandParameters);
            }
        }

        //public static DataSet ExecuteDataset(OracleConnection cn, CommandType commandType, string commandText, int pageSize, int pageNumber, OracleTransaction trx, params OracleParameter[] commandParameters)
        //{
        //    //create & open a OracleConnection, and dispose of it after we are done.
           

        //        //call the overload that takes a connection in place of the connection string
        //        return ExecuteDataset(cn, commandType, commandText, pageSize, pageNumber,trx, commandParameters);
        //}
        ...
        /// <param name="commandParameters">an array of SqlParamters used to execute the command</param>
        /// <returns>a dataset containing the resultset generated by the command</returns>
        public static DataSet ExecuteDataset(OracleConnection connection, CommandType commandType, string commandText, int pageSize, int pageNumber, IDbTransaction trx, params OracleParameter[] commandParameters)
        {
            if (commandParameters != null)
            {
                for (int i = 0; i <= commandParameters.Length - 1; i++)
...
//                        WHERE r__ >= (((" + pageNumber + @"-1) * " + pageSize + @") + 1)";
            ...
            //create a command and prepare it for execution
            OracleCommand cmd = new OracleCommand();
            PrepareCommand(cmd, connection, (OracleTransaction)trx, commandType, commandText, commandParameters);
